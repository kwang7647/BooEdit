...
private:
    ...
    void deleteBuckets( Node** b, unsigned int cap);

    void printBuckets( Node** b, unsigned int cap);

    bool bucketIsEmpty(Node** b, unsigned int index);

    Node* findTail(Node* n);

    void hashMapCopy( Node** target, Node** source, unsigned int cap, unsigned int newCap, bool rehash);
    ...
    Node** buckets;
    unsigned int bucketSize; //  current size
    unsigned int bucketCap; // capacity
    unsigned int totalPairs; //total num of key/values
    bool rehashing;
